...
本プロジェクトでは複数キーワードのAND検索（例: 「生成AI 規制」）をサポートしていません。これは意図的な設計判断です。

各ソースの1回の取得件数はGoogleニュース30件・BlueSky 20件・はてブ数十件程度であり、ANDで絞り込むと母数が数件になります。この件数では後段の感情分析パイプライン（BERTアンサンブルによる統計集約・ソース間乖離検出・分析タイプ判定）が統計的に機能しません。

また、はてなブックマーク検索RSSはフレーズ検索（`"keyword"`）を前提としており、複数キーワードのAND検索に対する挙動が不安定です。

詳細な検討経緯は [DEVELOPMENT_INSIGHT.md](DEVELOPMENT_INSIGHT.md) の「10. AND検索を採用しない理由」を参照してください。
