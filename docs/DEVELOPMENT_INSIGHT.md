...
## 10. AND検索（複数キーワード絞り込み）を採用しない理由

### 課題

「生成AI 規制」のように複数キーワードをAND条件で絞り込む機能の要否を検討しました。

### 検討 — 業界標準ツールのアプローチ

ソーシャルリスニング・メディア分析ツール（Brandwatch、Meltwater、Talkwalker等）では、ブーリアン検索（AND/OR/NOT）は**数万〜数十万件のデータプールからの絞り込み**に使われる手法です。

本プロジェクトのデータ量は以下の通りです。

| ソース | 1回の取得件数 |
|--------|---------------|
| Googleニュース | 最大30件 |
| BlueSky | 最大20件 |
| はてなブックマーク | 上位5記事のコメント（数十件） |

### 判断 — 統計的に破綻する

AND検索を適用した場合、以下の問題が発生します。

#### 母数の崩壊

各ソース20〜30件の母数に対してANDで絞り込むと、残る記事は数件程度になります。この件数では：

- BERTアンサンブルのソフト投票による感情スコアの**統計的信頼性が消失**する
- ソース別ネットスコア（positive − negative）が1件の記事に左右される
- 分析タイプ判定の平均±1σ相対評価が**N数不足で機能しない**

#### ソース間乖離検出の無効化

本プロジェクトの核心機能は「ソース間の温度差を定量検出する」ことです。AND絞り込みで各ソース3〜5件になった場合、乖離値は偶然の変動と区別できません。

#### はてなブックマークAPIとの非互換

はてなブックマーク検索RSSはフレーズ検索（`"keyword"`）を前提としており、複数キーワードのAND検索に対する挙動が不安定です。さらに、タイトルフィルタリング（`keyword in entry.title`）との組み合わせで、ヒット数が極端に減少します。

### 代替として有効なアプローチ

複数キーワードの分析ニーズに対しては、AND絞り込みではなく以下が業界的に正しいアプローチです。

| アプローチ | 内容 | 本プロジェクトとの整合性 |
|-----------|------|------------------------|
| キーワード比較分析 | 各キーワードを独立に分析し、空気感の差分を比較 | 現行アーキテクチャの延長で実装可能 |
| 時系列トレンド | 同一キーワードの感情推移を可視化 | 履歴機能の拡張で実現可能 |

これらは母数を維持したまま多角的な分析を提供でき、本プロジェクトの「ソース間乖離の定量検出」という強みを活かせます。

### 結論

本プロジェクトの分析パイプラインは「限られた母数から統計的に意味のある指標を抽出する」設計です。AND検索による母数の削減は、この設計前提と根本的に矛盾するため採用しません。
