...
- **定量的な感情分析パイプライン**
  ...
  - ソースごとにNet Sentiment Score（positive比率 − negative比率、範囲: -1.0〜+1.0）を算出し、温度計UIで直感的に可視化
- **ソース間乖離の定量検出**
...
8. **母数を維持する検索設計** — AND検索（複数キーワード絞り込み）は意図的に非採用。各ソース20〜30件の母数に対してANDで絞ると統計集約が破綻するため、1キーワード×多ソース比較に特化（詳細: [docs/DEVELOPMENT_INSIGHT.md](docs/DEVELOPMENT_INSIGHT.md#10-and検索複数キーワード絞り込みを採用しない理由)）
